...
const COPY = {
  ...
  hub: {
    ...
    // catalogue, fetched by `node tools/build.mjs currencies`. They are empty
...
function ctaBlock(locale, copy, ctx, campaign, extra = '') {
  ...
  const badges = [
    ...
    param('huawei', huawei),
    param('app', SITE.appName),
  ].join(' ')
  return `<div class="cc-cta">
<img class="cc-cta-icon" src="/images/currency-converter/icon-pro.png" alt="${esc(SITE.appName)}" width="72" height="72" loading="lazy" decoding="async">
...
function renderHubPage({ locale, copy, alternates, switcher, allLocaleHubs, dates, appCurrencies }) {
  ...
  // when `node tools/build.mjs currencies` has not been run.
...
<span class="cc-pair-name">${esc(currencyName(base, locale.intl))} ${arrow(locale)} ${esc(currencyName(quote, locale.intl))}</span>
  ...
  const edition = (name, note, href, store) =>
    `<li><b>${esc(name)}</b> <span>${esc(note)}</span> <a href="${esc(href)}" target="_blank" rel="noopener" aria-label="${store}, ${esc(t(name, ctx))}">${store}</a></li>`
...
function generate({ check = false } = {}) {
  ...
  written.set(
    ...
    `<!-- GENERATED by tools/build.mjs — do not edit. -->\n${
      active.map(({ locale }) =>
        `<li><a href="${hubPath(locale)}" hreflang="${locale.hreflang}" lang="${locale.hreflang}" aria-label="${locale.name} — ${esc(SITE.appName)} currency pair pages">${locale.name}</a></li>`).join('\n')
    }\n`,
  )
...
/**
 * The bounding box of the visible pixels in an 8-bit RGBA PNG.
 *
 * Google does not ship one badge artwork: the English file carries 41px of
 * transparent margin on every side, the French one none at all horizontally.
 * Rendered at the same height their black buttons come out 45.7px and 52.2px
 * tall, which is what made the row ragged from one language to the next. The
 * import trims each file to this box so every badge is ink to its edges and
 * one CSS height fits them all.
 */
function pngInkBox(file) {
  const buf = fs.readFileSync(file)
  if (buf.readUInt32BE(0) !== 0x89504e47) throw new Error(`not a PNG: ${file}`)

  let pos = 8
  let width = 0, height = 0, colour = -1
  const idat = []
  let alphaOfIndex = null
  while (pos < buf.length) {
    ...
    if (type === 'IHDR') {
      ...
      colour = data[9]
      // Google ships the English badge as a palette PNG and the others as RGBA.
      if (data[8] !== 8 || (colour !== 6 && colour !== 3) || data[12] !== 0) {
        throw new Error(`${file}: unsupported PNG (depth ${data[8]}, colour ${colour}, interlace ${data[12]})`)
      }
    } else if (type === 'tRNS') {
      alphaOfIndex = Buffer.from(data)
    } else if (type === 'IDAT') {
  ...
  const bytesPerPixel = colour === 6 ? 4 : 1
  ...
  const alphaAt = (y, x) => colour === 6
    ? out[y * stride + x * 4 + 3]
    : (alphaOfIndex && alphaOfIndex[out[y * stride + x]] !== undefined ? alphaOfIndex[out[y * stride + x]] : 255)

  let top = height, bottom = -1, left = width, right = -1
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      if (alphaAt(y, x) <= 16) continue
      if (y < top) top = y
  ...
  return { width, height, top, left, inkWidth: right - left + 1, inkHeight: bottom - top + 1 }
}

/** Crops a PNG down to its visible pixels. sips only crops from the centre, so
 *  a file whose margins are lopsided is left alone rather than shifted. */
function trimPng(file) {
  const box = pngInkBox(file)
  if (box.inkWidth === box.width && box.inkHeight === box.height) return null
  const evenX = box.left === box.width - box.left - box.inkWidth
  const evenY = box.top === box.height - box.top - box.inkHeight
  if (!evenX || !evenY) {
    console.warn(`! ${path.basename(path.dirname(file))}/${path.basename(file)} has off-centre margins, left as downloaded`)
    return null
  }
  sips(['-c', String(box.inkHeight), String(box.inkWidth), file, '--out', file])
  return box
}
  ...
  for (const locale of WEB_LOCALES) {
    ...
    for (const [name, url] of [
      ...
      try {
        ...
        let size = await download(url, dest)
        if (name.endsWith('.png') && trimPng(dest)) size = fs.statSync(dest).size
        total += size
 ...
 *
 *   CURRENCY_API_PASS=… node tools/build.mjs currencies
 *
...
async function fetchCurrencies() {
  if (!PASS) {
    ...
    console.error('Run:  CURRENCY_API_PASS=… node tools/build.mjs currencies')
    process.exit(1)
  }
 ...
 *
 * GENERATED by 'node tools/build.mjs currencies' from the app's own backend
 ...
 *
 *   node tools/build.mjs screenshots [--fastlane <path>] [--force]
 *
